...
@section('content')
    ...
    <div class="card border">
        <div class="card-body">
            <form method="get" action="{{ route('moderator.articles.index') }}" class="form-inline">
                ...
                <div class="form-group">
                    <a href="{{ route('moderator.articles.index') }}" class="btn btn-link btn-sm'">{{__('Reset')}}</a>
                </div>
        ...
        <div class="card-header">
            <h5 class="m-0">
                ...
                {{-- <button class="btn btn-primary btn-sm float-right"
                        onclick="window.location.href='{{ route('moderator.articles.create') }}'">
        ...
        <div class="card-body p-0">
            ...
            <div class="table-responsive">
                <table class="table table-striped">
                    ...
                    <tr>
                        <th>
                            {!! link_to_route('moderator.articles.index', __('Id'),
                                array_merge(request()->query(), ['order' => 'id', 'dir' => $orderBy['dir'], 'page' => 1]) ) !!}
                        </th>
                        <th>
                            {!! link_to_route('moderator.articles.index', __('Title'),
                                array_merge(request()->query(), ['order' => 'title', 'dir' => $orderBy['dir'], 'page' => 1]) ) !!}
                        </th>
                        <th>
                            {!! link_to_route('moderator.articles.index', __('Slug'),
                        ...
                        <th>{{ __('Updated') }}</th>
                        <th>
                            {!! link_to_route('moderator.articles.index', __('Published'),
                            array_merge(request()->query(), ['order' => 'published_at', 'dir' => $orderBy['dir'], 'page' => 1]) ) !!}
                        </th>
                        <th>
                            {!! link_to_route('moderator.articles.index', __('Created'),
                    ...
                    @foreach ($articles as $article)
                        <tr>
                            <td>{{ $article->id }}</td>
                            <td>
                                <a href="{{ route('moderator.articles.edit', [$article->id]) }}">{{ $article->title }}</a>
                            </td>
                            <td>{{ $article->slug }}</td>
