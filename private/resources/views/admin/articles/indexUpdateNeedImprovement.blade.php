...
@section('content')
    ...
    <div class="card border">
        <div class="card-body">
            <form method="get" action="{{ route('admin.articles.indexUpdateNeedImprovement') }}" class="form-inline">
                ...
                <div class="form-group">
                    <a href="{{ route('admin.articles.indexUpdateNeedImprovement') }}"
        ...
        <div class="card-body p-0">
            ...
            <div class="table-responsive">
                <table class="table table-striped">
                    ...
                    <tr>
                        <th>
                            {!! link_to_route('admin.articles.indexUpdateNeedImprovement', __('Id'),
                                array_merge(request()->query(), ['order' => 'id', 'dir' => $orderBy['dir'], 'page' => 1]) ) !!}
                        </th>
                        <th>
                            {!! link_to_route('admin.articles.indexUpdateNeedImprovement', __('Title'),
                                array_merge(request()->query(), ['order' => 'title', 'dir' => $orderBy['dir'], 'page' => 1]) ) !!}
                        </th>
                        <th>
                            {!! link_to_route('admin.articles.indexUpdateNeedImprovement', __('Slug'),
                                array_merge(request()->query(), ['order' => 'slug', 'dir' => $orderBy['dir'], 'page' => 1]) ) !!}
                        </th>
                        <th>{{ __('Author') }}</th>
                        <th>{{ __('Updated') }}</th>
                        <th>
                            {!! link_to_route('admin.articles.indexUpdateNeedImprovement', __('Published'),
                            array_merge(request()->query(), ['order' => 'published_at', 'dir' => $orderBy['dir'], 'page' => 1]) ) !!}
                        </th>
                        <th>
                            {!! link_to_route('admin.articles.indexUpdateNeedImprovement', __('Created'),
                    ...
                    @foreach ($articles as $article)
                        <tr>
                            <td>{{ $article->id }}</td>
                            <td>
                                <a href="{{ route('admin.articles.updateNeedImprovementEdit', [$article->id]) }}">{{ $article->title }}</a>
                            </td>
                            <td>{{ $article->slug }}</td>
